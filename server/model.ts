...
export function areCompatible(card: Card, lastCardPlayed: Card) {
  if(lastCardPlayed) {
    return card.rank === lastCardPlayed.rank || card.suit === lastCardPlayed.suit || card.rank === "K" || lastCardPlayed.rank === "K"
  } else {
   return false
  }
}
...
/**
 * determines if someone is close to winning the game game -- i.e., has 2 or fewer cards in their hand
 */
export function nearWinners(state: GameState): number[] {
  let counts = computePlayerCardCounts(state)
  let nearWinners : number[] = []

  counts.forEach((count, index) => {
    if(count <= 2) {
      nearWinners.push(index)
    }
  })
